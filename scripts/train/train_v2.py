...
#read text
raw_text = readtxt("llm/data/train-simplestory.txt")


#Open configs
with open("llm/src/Architecture/MODEL_CONFIGS.json", "r", encoding="utf-8") as f:
    cfg = json.load(f)
...
if __name__ == "__main__":
    ...
    eval_freq=5, eval_iter = 5, start_context="Hello, one day, I will", tokenizer=tiktoken.get_encoding("gpt2"))

    epochs_tensor = torch.linspace(0, num_epochs, len(train_loss))
    plot_losses(epochs_tensor, tokens_seen, train_loss, val_loss)
    torch.save(model.state_dict(), "checkpoints/xperyv3.pth")
    exit(0)
